...
export class CrosswordComponent implements OnInit, WebComponentHooks<CrosswordState> {
  ...
  ngOnInit(): void {
    ...
    this.state.words.forEach((element) => {this.words.push(element)});
  ...
  generateGrid(){
    ...
    let firstInit = this.state.grid.length == 0;// user can change results or grid but must not to it
    ...
    if (firstInit) {
      this.userAnswers.forEach(res => {this.state.results.push({answer:res.answer,number:res.position})})
      this.state.grid = [];
      ...
      this.userAnswers.forEach(res =>  this.state.results.filter(el => el.number == res.position).map(el => res.answer = el.answer));
      this.color = true;
    }
...
//    this.state.grid.forEach(line => this.nbCaracterToFill += line.filter(cell => cell.charAt(0) != '-').length)
  ...
  focusNextCell(x : number, y : number, dir : number) {
    ...
    if(result) {
      const nextCoordonate = this.crossWordService.coordonateRedirectionByTabulationService(result.orientation, result.answer.length, x, y, dir)
  ...
  onKeyDown(event: KeyboardEvent, x : number, y : number) {
    ...
    if (key === "Backspace") {
      ...
      var direction = this.crossWordService.directionTypeDelete(x,y);
      if (direction === "H") {
