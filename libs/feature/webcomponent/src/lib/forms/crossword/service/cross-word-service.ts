...
export class CrossWordService {
  ...
  directionTypeDelete(x:number, y:number) : string {
    var vertical = y - 1 >= 0 && this.grid[y-1][x] != '-';
    var horizontal = x - 1  >= 0 && this.grid[y][x-1] != '-';

    var isAdjacentY = (y + 1 == this.lastPosition[1] && x == this.lastPosition[0]);
    var isAdjacentX =(x + 1 == this.lastPosition[0] && y == this.lastPosition[1]);
  ...
  getInformationDirection(vertical: boolean, horizontal: boolean, isAdjacentY: boolean, isAdjacentX: boolean): string {
    if ((horizontal == vertical) && horizontal) {// crossing

      if (isAdjacentX || isAdjacentY) { // keep the same direction
        return this.lastStatusPos;
      }
      return "U"; // click on a random crossing
    }
    if (horizontal == vertical) { // both are false
  ...
  */
  directionType(x:number, y:number) : string {
    var vertical = y + 1 < this.grid.length && this.grid[y+1][x] != '-';
    var horizontal = x + 1  < this.grid[0].length && this.grid[y][x+1] != '-';
  ...
  coordonateRedirectionByTabulationService(orientation : string, size : number, x : number, y : number, dir :number) : Coordonate{
    let nextX = x;
    let nextY = y;
    ...
    switch (dir) {
      case -1:
        var direction = this.directionType(x,y);
        if (direction == "V") {
           nextY ++;
