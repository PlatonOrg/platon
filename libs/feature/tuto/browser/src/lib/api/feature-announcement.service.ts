...
interface AnnouncementDisplayStats {
...
export class FeatureAnnouncementService {
  ...
  public isAnnouncementVisible = signal(false);
  public currentAnnouncement = signal<FeatureAnnouncement | null>(null);

  private readonly STORAGE_KEY = 'platon-feature-announcements';
  ...
  private announcements: FeatureAnnouncement[] = [
    {
      ...
      actions: {
        ...
        secondary: {
          text: 'Plus tard',
          action: () => this.dismissAnnouncement('later')
        }
      }
    }
  ...
  private shouldShowAnnouncement(announcement: FeatureAnnouncement): boolean {
    ...
    if (stats.timesShown >= this.MAX_DISPLAY_COUNT) {
      return false;
    }

    // Vérifier si assez de temps s'est écoulé depuis le dernier affichage
    const daysSinceLastShown = (now - stats.lastShown) / (1000 * 60 * 60 * 24);
    if (daysSinceLastShown < this.MIN_INTERVAL_DAYS) {
      return false;
    }

    // Vérifier si on est dans la période totale d'affichage
    const daysSinceFirstShown = (now - stats.firstShown) / (1000 * 60 * 60 * 24);
    if (daysSinceFirstShown > this.TOTAL_PERIOD_DAYS) {
      return false;
    }
   ...
   * Ferme l'annonce
   */
  dismissAnnouncement(reason: 'close' | 'later' | 'interaction'): void {
  ...
  private openTutorialSelector(): void {
    ...
    this.dismissAnnouncement('interaction');
   ...
   */
  private recordAnnouncementDismissed(announcementId: string, reason: string): void {
    const stats = this.getAnnouncementStats(announcementId);

    if (reason === 'close') {
      stats.dismissed = true; // Ne plus jamais afficher
    }
    // Pour 'later', on ne marque pas dismissed pour permettre réaffichage
  ...
  /**
   * Méthodes utilitaires pour le debugging et l'administration
   */
   ...
   */
  getAllAnnouncementStats(): Record<string, AnnouncementDisplayStats> {
    const stored = localStorage.getItem(this.STORAGE_KEY);
   ...
   */
  forceShowAnnouncement(announcementId: string, user: User): void {
