...
export class RemoteCourseSectionProvider extends CourseSectionProvider {
  ...
  find(courseId: string, sectionId: string): Observable<CourseSection> {
    return this.http
      .get<ItemResponse<CourseSection>>(`/api/v1/courses/${courseId}/sections/${sectionId}`)
  ...
  create(course: Course, input: CreateCourseSection): Observable<CourseSection> {
    return this.http
      .post<ItemResponse<CourseSection>>(`/api/v1/courses/${course.id}/sections`, input)
      .pipe(map((response) => response.resource))
  }
  ...
  delete(section: CourseSection): Observable<void> {
    return this.http.delete<void>(`/api/v1/courses/${section.courseId}/sections/${section.id}`)
  }
}
