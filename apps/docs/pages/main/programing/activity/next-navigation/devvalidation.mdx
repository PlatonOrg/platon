...
4. **Débogage** : Si vous avez besoin de tracer des informations ou de déboguer votre logique d'évaluation, vous pouvez utiliser `platon_log`. Pour cela, injectez de façon sécurisée la fonction `platon_log` au tout début de votre fichier :
   ```python
   import builtins
   platon_log = getattr(builtins, "platon_log", print)
   ```
5. **Le cas par défaut (Obligatoire)** : Vous devez impérativement inclure à la fin de la fonction un cas par défaut renvoyant `None, None`. Cela permet de basculer automatiquement sur le comportement standard de la plateforme pour tous les groupes d'exercices n'ayant pas besoin de règles spécifiques.
