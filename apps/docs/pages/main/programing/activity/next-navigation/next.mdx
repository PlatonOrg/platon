...
- **Passer automatiquement** : Active ou désactive le passage automatique à l'exercice suivant dès que l'utilisateur termine celui en cours.
  Passer automatiquement signifie que lorsque l'utilisateur valide l'exercice avec une note supérieure à une valeur minimale, il est redirigé vers l'exercice suivant sans avoir à cliquer sur la flèche.
...
- **Navigation libre** : Permet à l'utilisateur de retourner à un exercice précédent.
- **Variables d'exercices** : Permet d'avoir accès en lecture seule aux variables de tous les exercices dans le *Next*.

<Callout type="info">Les options `Passer automatiquement` et `Note minimum pour passer automatiquement` impactent uniquement le passage **automatique** des exercices.
...
Un guide pour programmer le *Next* est disponible [ici](/main/programing/activity/next-navigation/devnext).

<Callout type="info">Une bibliothèque contenant des fonctions de base est disponible.
Vous pouvez consulter la liste complète des fonctions utilisables pour programmer le *Next* en [cliquant ici](/main/programing/activity/next-navigation/nextlib).</Callout>
