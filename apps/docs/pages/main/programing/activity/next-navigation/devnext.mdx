...
Une documentation complète des fonctions disponibles dans la NextLib est disponible [ici](/main/programing/activity/next-navigation/nextlib).</Callout>
...
# Empêche de passer à l'exercice suivant si l'actuel n'a pas été essayé
playCurrentIfUnplayed()
...
3. **Faire l'exercice en cours :** La fonction `playCurrentIfUnplayed()` est utilisée pour éviter de lancer un nouvel exercice si l'étudiant n'a pas encore essayé l'exercice en cours.
...
<Callout type="info">D'autres fonctions sont disponibles pour lancer des exercices, comme `playCurrentIfUnplayed()` ou `playAllFromGroup(groupNb: int, randomOrder: bool = False)`.
...
if current and getCurrentGrade() < 50:
...
bestGrade = getExerciseBestGrade(current)
attempts = getExerciseAttempts(current)
...
La stratégie de notation est une fonction qui retourne un entier, correspondant à la note finale de l'activité.
...
<Callout type="info">Il est possible de stocker des données de tout type, comme des nombres, des chaînes de caractères, des listes, des dictionnaires, etc.</Callout>
...
if exercisesToDo:
    ...
    # Lance le premier exercice de la liste et le retire
...
Pour faciliter le débogage du Next, il est possible d'utiliser la fonction `platon_log(message: str)`.
